...
const Profile: React.FC<IProfileProps> = ({ Member }): JSX.Element => {
  ...
  return (
    <div
      className={`w-[36px] h-full ml-[-8px] last:ml-[0px] pt-[9px] pb-[7px] px-xs rounded-full bg-${Member.userColor}-Secondary flex justify-center items-center`}
    >
      {Member.userImage?.length ? (
        <img src={Member.userImage} />
      ) : (
        <p
          className={`font-IranYekan400 text-BodyXS text-${Member.userColor}-Primary`}
        >
